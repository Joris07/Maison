...
def trier(nbr, chaine):
    ...
    memo = read_file().split(";") #Récupération de la derniére valeur publier
    if(tab[0]!=memo[0]): #Si la valeur reçu est différente de la derniére publier alors
        for i in range(1, nbr+1): #Parcours toutes les cases du tableau en commencant à la 2éme
            if(tab[i].find("t")!=-1): #Si la case contient un t alors  
                ...
                client.publish(tab[0][1:]+"/humidite",tab[i][1:]) #Publie l'humidite du topic sur le broker       
